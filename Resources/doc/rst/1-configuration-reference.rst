...
see `"Extending Authenticator" <6-extending-jwt-authenticator>`__

Database-less User Provider
'''''''''''''''''''''''''''

For a database-less authentication (i.e. trusting into the JWT data
instead of reloading the user from the database), see `"A database less
user provider" <8-jwt-user-provider>`__.
